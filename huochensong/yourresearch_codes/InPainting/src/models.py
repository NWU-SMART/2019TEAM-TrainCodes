...
from .networks import InpaintGenerator, EdgeGenerator, Discriminator,BaseNetwork
from .dataset import Dataset
from .loss import AdversarialLoss, PerceptualLoss, StyleLoss
from .utils import get_model_list
from .old_networks import EdgeGenerator as OldNet
...
class BaseModel(nn.Module):
    def __init__(self, name, config):
        ...
        self.config = config
        self.iteration = 0
        # self.config.PATH = r'F:/pythoncode/anime/Anime-InPainting/model/getchu/'
        # self.config.PATH  = './model/getchu/'
    ...
    def load(self):
        ...
        if gen_path is not None:
            ...
            # my_model = EdgeGenerator()
            # print(type(my_model),type(data))
            # pretrained = OldNet()
            # pretrained.load_state_dict(data['generator'])
...
class EdgeModel(BaseModel):
    ...
    def process(self, images, edges, masks):
        ...
        dis_loss1 = dis_loss +  (dis_real_loss + dis_fake_loss) / 2
        ...
        for i in range(len(dis_real_feat)):
            gen_fm_loss2 =gen_fm_loss1 + self.l1_loss(gen_fake_feat[i], dis_real_feat[i].detach())
        gen_fm_loss = gen_fm_loss2 * self.config.FM_LOSS_WEIGHT
        gen_loss2 = gen_loss1 + gen_fm_loss
        ...
        return outputs, gen_loss2, dis_loss1, logs
